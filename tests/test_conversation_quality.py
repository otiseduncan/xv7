...
def test_generate_website_prompt_emits_preview_code_artifact_payload(
    ...
    assert (
        ...
        not in str(message.get("content") or "").lower()
    )
...
def test_harry_site_revision_prompt_updates_existing_artifact_payload(
    monkeypatch, tmp_path: Path
) -> None:
    client = _setup_contract_only(monkeypatch, tmp_path)
    session_id = _new_session(client)
    ...
    revise = client.post(
        ...
        headers={"X-XV7-API-Key": "test-secret"},
        json={
            "raw_text": "Make this site look more premium and add a Specials section."
        },
    )
...
def test_preview_style_prompt_stays_in_chat_artifact_mode_without_sandbox_write(
    monkeypatch, tmp_path: Path
) -> None:
    client = _setup_contract_only(monkeypatch, tmp_path)
...
def test_natural_language_build_prompt_stays_chat_artifact_until_explicit_sandbox_write(
    monkeypatch, tmp_path: Path
) -> None:
    client = _setup_contract_only(monkeypatch, tmp_path)
    sandbox_root = tmp_path / "sandbox"
...
def test_sandbox_build_followup_revision_updates_latest_sandbox_target(
    monkeypatch, tmp_path: Path
) -> None:
    client = _setup_contract_only(monkeypatch, tmp_path)
    sandbox_root = tmp_path / "sandbox"
    ...
    assert create.status_code == 200
    create_artifact = create.json()["messages"][-1]["metadata"].get("code_artifact", {})
    assert create_artifact.get("delivery_mode") == "chat_artifact"

    write = client.post(
        f"/sessions/{session_id}/messages",
        headers={"X-XV7-API-Key": "test-secret"},
        json={"raw_text": "Write it to the sandbox."},
    )
    assert write.status_code == 200
    write_artifact = write.json()["messages"][-1]["metadata"].get("code_artifact", {})
    target_path = Path(str(write_artifact.get("sandbox_target_path") or ""))
