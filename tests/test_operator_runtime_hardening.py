...
def test_repo_status_parses_branch_sync_and_status(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def test_runtime_health_uses_internal_service_urls(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("core.operator.actions.runtime._get_json", lambda _url: (True, {"status": "ok"}))
    monkeypatch.setattr("core.operator.actions.runtime._probe_url", lambda _url, timeout=6: (True, 200, None))
...
def test_docker_compose_ps_availability_check_no_fake_proof(
    ...
    assert "No action was run beyond the read-only availability check" in result.stderr_summary
